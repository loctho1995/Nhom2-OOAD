    ...
    $('#avatar-upload-form').ajaxForm({
        ...
        success: function (data) {
            ...
            if (data.success === false) {
                ...
                $('#Avatar').val(data.fileName.replace("\\Temp\\", ""));
                $('#btnCreate').attr("onclick", "saveAvatar(); return ValidateEmployee();");
...
function updatePreviewPane(c) {
    ...
    if (($(".searchResult").is(':hidden'))) {
        $(".sidebar-nav.navbar-collapse.col-md-3.left_col").css("min-height", $(".x_panel").height() + $(".graphs").height() + 90);
    }
}
